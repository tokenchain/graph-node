...
pub fn api_schema(input_schema: &Document) -> Result<Document, APISchemaError> {
    ...
    add_changed_block_filter_input_type(&mut schema);
...
/// Adds a global `BlockChangedFilter` type to the schema. The `block` argument
/// accepts values of this type
fn add_changed_block_filter_input_type(schema: &mut Document) {
    let typedef = TypeDefinition::InputObject(InputObjectType {
        position: Pos::default(),
        description: None,
        name: CHANGE_BLOCK_FILTER_NAME.to_string(),
        directives: vec![],
