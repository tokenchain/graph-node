...
fn parse_change_block_filter(
    value: &graph::data::value::Value,
) -> Result<BlockNumber, QueryExecutionError> {
    match value {
        graph::data::value::Value::Object(object) => {
            let block_value = object
                .get("number_gte")
                .ok_or_else(|| QueryExecutionError::InvalidFilterError)?;

            match block_value {
                graph::data::value::Value::Int(n) => Ok(*n as i32),
                _ => Err(QueryExecutionError::InvalidFilterError),
            }
        }
        _ => Err(QueryExecutionError::InvalidFilterError),
